        ...
        <nav className='sm:flex justify-evenly min-w-1/2 hidden h-full items-center'>
          {
            navItems.map((a, b) =>
              <button key={b}
                ...
                ${currentSection === a.name ? "text-primary" : theme.text}`}
                onClick={() => { scrollPage(a.ref) }}
              >{a.name}</button>)
          }
        </nav>
        <button onClick={changeTheme} className="cursor-pointer p-2">
          {theme.text === colors.text ? <MdOutlineLightMode size={25} /> : <MdOutlineDarkMode size={25} />}
        </button>
